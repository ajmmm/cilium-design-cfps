...
The original implementation of CLRP added loop prevention to the datapath using socket lookups (`bpf_sk_lookup_*` helpers). The PR discussion noted that this depended on the relevant `sk_lookup` helpers being available, but did not expose a per-policy opt-out. Listening sockets that happened to match the destination port could cause translation to be skipped incorrectly, as reported in [#15195](https://github.com/cilium/cilium/issues/15195) and [#20164](https://github.com/cilium/cilium/issues/20164). [PR #26144](https://github.com/cilium/cilium/pull/26144) replaced the socket-lookup implementation with netns-cookie-based skip maps and introduced `skipRedirectFromBackend` as an opt-in field. The linked history does not document why that behaviour changed.

During the review of [PR #13287](https://github.com/cilium/cilium/pull/13287), the possibility of redirecting traffic from an LRP backend back to that same backend was considered, but no current use case was identified. The available history does not identify a subsequent use case. Based on that, loop prevention should be the only supported behaviour for CCLRP: CCLRP should not expose an opt-out. The agent should enable loop prevention for all CCLRPs when the required kernel capability is available; otherwise, it should log a warning that loop prevention is unavailable.

*Note: `skipRedirectFromBackend` will remain opt-in for CLRP.*

The skip maps are a separate concern. The netns-cookie implementation uses separate IPv4 and IPv6 maps, each currently limited to 100 entries ([map definitions](https://github.com/cilium/cilium/blob/debffba0a964b463d90855ac97a911fd672af6ef/pkg/loadbalancer/maps/types.go#L1266-L1275)). A skip map entry is required for each combination of an eligible backend network namespace and matching frontend address and port in that address family. For the examples in this CFP, a single backend with two frontend address/port combinations, such as ports 80 and 443, would consume two entries in the relevant map. Multiple backend network namespaces, frontend address/port combinations, or CCLRPs increase usage linearly. The implementation should evaluate a realistic worst-case configuration for the intended CCLRP use cases and determine whether the current limit is sufficient. It must not silently enable a CCLRP without loop prevention if skip map capacity is exhausted.

* Original socket lookup implementation: [PR #13287](https://github.com/cilium/cilium/pull/13287)
* Revised "skip map" implementation: [PR #26144](https://github.com/cilium/cilium/pull/26144)
...
A prototype implementation will be used to validate the API and reconciliation behaviour with tests covering address matching, Service matching, namespace-restricted selectors, explicit port translation, invalid configuration, backend churn, nodes without local backends preserving the original frontend behaviour, kernel capability handling, and skip map entry usage and capacity limits.
